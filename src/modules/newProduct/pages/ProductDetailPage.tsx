...
import { IUpdateProductParams } from "../../../models/products";
import { AppState } from "../../../redux/reducer";
import { getErrorMessageResponse } from "../../../utils";
import { addNotification } from "../../common/redux/notificationReducer";
import { fetchThunk } from "../../common/redux/thunk";
import ProductDetailUpdateForm from "../components/ProductDetailUpdateForm";
import {
  setBrands,
  setCategories,
  setConditions,
...
const ProductDetailPage = (props: Props) => {
  ...
  const getProductDetail = useCallback(async () => {
    ...
    const json = await dispatch(
      fetchThunk(API_PATHS.productDetail, "post", { id })
    );
    ...
    dispatch(
      addNotification({ message: getErrorMessageResponse(json), type: "error" })
    );
  }, [dispatch, id]);
  const onUploadFile = useCallback(
    async (file: File, productId: string, order: string) => {
      ...
      dispatch(
        addNotification({
          message: getErrorMessageResponse(json),
          type: "error",
        })
      );
    },
      ...
      if (!json?.errors) {
        ...
        await Promise.all(
          files.map(async (file) => {
            const index = values.imagesOrder.findIndex(
              (item) => item === file.name
            );
            if (index !== -1) {
