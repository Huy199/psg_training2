...
const UserFilter = (props: Props) => {
  ...
  const membershipTypes = {
    memberships: [{ label: "general", value: "M_4" }],
    pendingMemberships: [{ label: "general", value: "P_4" }],
  };
  const statuses = [
  ...
  const handleTypesChange = (e: any) => {
    setFilter({
      ...filter,
      types: [...e.target.value].filter((type) => type !== "All user types"),
    });
  };
  const handleMembershipsChange = (e: any) => {
    setFilter({
      ...filter,
      memberships: [...e.target.value].filter(
        (type) => type !== "All memberships"
      ),
    });
  };
  ...
  useEffect(() => {
    ...
    const country = [...countries].find(
      (country) => country.country === filter.country
    );
    if (!country) {
      return;
    }
      ...
      <Grid container width={1} sx={{ borderBottom: "1px solid inherit" }}>
        ...
        >
          ...
          >
              ...
              >
                ...
                {membershipTypes.memberships.map((type) => (
                  <MenuItem key={type.label} value={type.value}>
                    <Checkbox
                      checked={filter.memberships.indexOf(type.value) > -1}
                    />
                    <ListItemText
                      sx={{ textAlign: "left" }}
                      primary={type.label}
            ...
            <FormControl
              size="small"
              sx={{ m: 1, width: "20%", color: "#fff", ...styledForField }}
            >
              <InputLabel>User types</InputLabel>
              <Select
                multiple
                value={
                  filter.types.length === 0 ? ["All user types"] : filter.types
                }
                  ...
                  selected.map((type, index) => {
                    switch (type) {
                      ...
                      case "4":
                        return "Vendor ";
                      case "5":
                        return "View order reports ";
                      case "6":
                        return "Volume discounts management ";
                      case "C":
              ...
              >
                ...
                {userTypes?.administrator.map((type) => (
                  <MenuItem key={type.name} value={type.id}>
                    <Checkbox checked={filter.types.indexOf(type.id) > -1} />
              ...
              <Box width={1} p={1} sx={{ display: "flex" }}>
                ...
                >
                  <FormControl
                    size="small"
                    fullWidth
                    sx={{ m: 1, color: "#fff", ...styledForField }}
                  >
                  ...
                  {cloneStates && cloneStates.length > 0 && true ? (
                    <FormControl
                      size="small"
                      fullWidth
                      sx={{ m: 1, color: "#fff", ...styledForField }}
